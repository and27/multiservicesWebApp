      ...
      database.once("value", function(snapshot) {
        snapshot.forEach(function(child) {
          ...
          if (child.key=="pic1"){
           ...
           nombre= nombre+"<p  onclick=f('"+ useruid+"')> Publicado por "+JSON.stringify(child).split('"').join('')+"</p>";
          }
          ...
          else if(child.key=="star"){
            star="<p> Calificacion: "+JSON.stringify(child).split('"').join('')+"</p> ";
          }
        ...
        text=text+imagenes+title+desc+nombre+star+"</div>"+valor;
